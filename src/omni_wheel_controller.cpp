...
// In Jazzy, get_state() was removed from ControllerInterfaceBase
// and LoanedStateInterface/LoanedCommandInterface APIs changed
...
namespace omni_wheel_controller
{
  ...
  controller_interface::CallbackReturn OmniWheelController::on_init()
  {
    try
    {
      ...
      // Use NodeParametersInterface directly to avoid --params-file parsing issues in Jazzy
      ...
      const rclcpp::Time &time, const rclcpp::Duration &period)
  {
    ...
    // In Jazzy+, get_state() was removed. Use get_lifecycle_state() instead.
...
#if OMNI_WHEEL_ROS2_JAZZY_OR_LATER
    received_velocity_msg_ptr_.get([&last_command_msg](const std::shared_ptr<Twist> &msg)
    ...
    else
    {
      ...
      {
#if OMNI_WHEEL_ROS2_JAZZY_OR_LATER
        auto value_opt = registered_omni_wheel_handles_[index].feedback.get().get_optional();
        if (!value_opt.has_value() || std::isnan(value_opt.value()))
        {
          RCLCPP_ERROR(
        ...
        wheel_feedback.push_back(value_opt.value());
#else
    ...
    {
#if OMNI_WHEEL_ROS2_JAZZY_OR_LATER
      // In Jazzy+, set_value returns bool and is nodiscard
      (void)registered_omni_wheel_handles_[index].velocity.get().set_value(wheel_movement_vector(index));
#else
...
#if OMNI_WHEEL_ROS2_JAZZY_OR_LATER
    received_velocity_msg_ptr_.set([&empty_twist](std::shared_ptr<Twist> &msg)
...
#if OMNI_WHEEL_ROS2_JAZZY_OR_LATER
            received_velocity_msg_ptr_.set([&msg](std::shared_ptr<Twist> &stored_msg)
...
#if OMNI_WHEEL_ROS2_JAZZY_OR_LATER
                received_velocity_msg_ptr_.set([this, &msg](std::shared_ptr<Twist> &twist_stamped)
                                               {
            if (twist_stamped) {
...
#if OMNI_WHEEL_ROS2_JAZZY_OR_LATER
    received_velocity_msg_ptr_.set([](std::shared_ptr<Twist> &msg)
  ...
  void OmniWheelController::halt()
  {
    ...
    {
#if OMNI_WHEEL_ROS2_JAZZY_OR_LATER
      (void)wheel_handle.velocity.get().set_value(0.0);
#else
